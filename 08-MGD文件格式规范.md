...
### 8.1 主题 id 空间（内置 + 用户自建风格）

`Document.theme.id` 是**字符串**，取值来自两处：

- **内置主题**（`themes/built-in-themes.ts`，17 套）：`classic-blue` / `dark` / … / `innocent`。
  这些 id 是**字面量联合**（`BuiltinThemeId`），改一个等于改外观契约。
- **用户自建风格**（本机风格库）：`custom-theme-` 前缀，运行时生成
  （XMind「工具 → 创建自定义风格」的对应物）。风格库**不在 .mgd 里**，
  它存在本机（`localStorage` 的 `mindgrid:custom-themes`），随机器走、不随文件走。

因此 **.mgd 里完全可能记着一个本机没有的主题 id**（把文件发给同事就会遇到）。
读取端必须：

1. **回落到默认主题**渲染（不能崩、不能白屏）；
2. 在「文档主题」处**显式提示**该 id 在本机不存在 —— 否则用户只看到"配色不对"，
   根本不知道是缺了一份风格（`isThemeResolvable` 就是给这个提示用的）。

渲染取主题**只能走一个入口**：`lib/document/themes` 导出的 `getTheme`（内置 + 自定义合并查询）。
内置表的查询函数刻意命名为 `getBuiltInTheme` —— 让"漏查自定义风格"这件事在命名上就说不通
（那类缺陷没有任何报错，只是"选了风格却不生效"）。

自建风格的**存储形态只认不透明 `#rrggbb`**：内置主题里大量使用 `rgba()` 与 `transparent`
（描边、连线、"没有边框"），进入编辑器时按不透明度与底色**合成为不透明色**
（`toOpaqueHex`）。不这么做会有两个后果：取色控件显示成黑色（它拿到非法值会回落 `#000000`），
以及保存时整条风格被校验丢弃 —— 用户看到的是"点了保存什么都没发生"。
